...
export const RootStoreBase = MSTGQLStore
  ...
  .actions(self => ({
    ...
    queryDupa(variables?: {  }, resultSelector: string | ((qb: MessageModelSelector) => MessageModelSelector) = messageModelPrimitives.toString(), options: QueryOptions = {}) {
      return self.query<{ dupa: MessageModelType}>(`query dupa { dupa {
        ${typeof resultSelector === "function" ? resultSelector(new MessageModelSelector()).toString() : resultSelector}
      } }`, variables, options)
    },
    mutateAddUser(variables: { name: string, avatar: string }, resultSelector: string | ((qb: UserModelSelector) => UserModelSelector) = userModelPrimitives.toString(), optimisticUpdate?: () => void) {
      return self.mutate<{ addUser: UserModelType}>(`mutation addUser($name: String!, $avatar: String!) { addUser(name: $name, avatar: $avatar) {
        ${typeof resultSelector === "function" ? resultSelector(new UserModelSelector()).toString() : resultSelector}
